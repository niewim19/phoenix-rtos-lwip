	...
	u32_t tunnel_dest, spd_table *table, int sort_src_first)
{
	...
	if (LIST_IS_EMPTY(table))
		...
		mask_len = sort_src_first ? __builtin_ffs(lwip_ntohl(n->src_mask)) : __builtin_ffs(lwip_ntohl(n->dest_mask));
		mask_len2 = sort_src_first ? __builtin_ffs(lwip_ntohl(n->dest_mask)) : __builtin_ffs(lwip_ntohl(n->src_mask));
		port = sort_src_first ? src_port : dst_port;
		port2 = sort_src_first ? dst_port : src_port;

		LIST_FOR_EACH(table, entry, list)
		{
			entry_mask_len = sort_src_first ? __builtin_ffs(lwip_ntohl(entry->src_mask)) : __builtin_ffs(lwip_ntohl(entry->dest_mask));
			if (mask_len < entry_mask_len)
				break;
			if (mask_len > entry_mask_len)
				continue;

			entry_mask_len = sort_src_first ? __builtin_ffs(lwip_ntohl(entry->dest_mask)) : __builtin_ffs(lwip_ntohl(entry->src_mask));
