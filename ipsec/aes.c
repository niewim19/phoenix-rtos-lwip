 ...
 */
static int aes_setKeyEnc(unsigned int rk[], const unsigned char cipherKey[], int keyBytes)
{
	int i = 0;
	unsigned int temp;
 ...
 */
static int aes_genKeySched(unsigned int rk[], unsigned int rrk[], const unsigned char cipherKey[], int keyBytes)
{
	int Nr, i;

	// expand the cipher key
	Nr = aes_setKeyEnc(rk, cipherKey, keyBytes);
 ...
 */
static void aes_encBlk(AES_ctx_t *pCtx, const unsigned char pt[], unsigned char ct[])
{
 ...
 */
static void aes_decBlk(AES_ctx_t *pCtx, const unsigned char ct[], unsigned char pt[])
{
...
int AES_init(AES_ctx_t *ctx, unsigned char *pIV, unsigned char *pKey, size_t KeyLen, int Mode)
{
	if (pKey == 0 || ctx == NULL || (KeyLen != AES_128 && KeyLen != AES_192 && KeyLen != AES_256))
		return -1;

	ctx->Nr = aes_genKeySched(ctx->Ek, ctx->Dk, pKey, KeyLen);

	if (pIV != 0) {
		ctx->Iv[0] = GETU32(pIV);
		ctx->Iv[1] = GETU32(pIV + 4);
		ctx->Iv[2] = GETU32(pIV + 8);
		ctx->Iv[3] = GETU32(pIV + 12);
	}
...
int AES_encrypt(AES_ctx_t *ctx, unsigned char *plain, unsigned char *cipher, size_t len)
{
	size_t i;

	if (plain == 0 || cipher == 0 || ctx == 0 || (len & 0xf) != 0)
		return -1;

	for (i = 0; i < len; i += AES_BLOCKSZ) {
		// encrypt block by block
		aes_encBlk(ctx, plain, cipher);
		cipher += AES_BLOCKSZ;
		plain += AES_BLOCKSZ;
	}
	return len;
...
int AES_decrypt(AES_ctx_t *ctx, unsigned char *cipher, unsigned char *plain, size_t len)
{
	size_t i;
	...
	const uint8_t *iv, const int mode, uint8_t *output)
{
	...
	ctx.Nr = aes_genKeySched(ctx.Ek, ctx.Dk, key, key_len);
	ctx.Iv[0] = GETU32(iv);
	ctx.Iv[1] = GETU32(iv + 4);
	ctx.Iv[2] = GETU32(iv + 8);
	ctx.Iv[3] = GETU32(iv + 12);
	ctx.Mode = mode & ~IPSEC_CIPHER_ENCRYPT;

	if (mode & IPSEC_CIPHER_ENCRYPT)
		aes = aes_encBlk;
	else
		aes = aes_decBlk;

	for (i = 0; i < text_len; i += AES_BLOCKSZ) {
		aes(&ctx, text, output);
		text += AES_BLOCKSZ;
		output += AES_BLOCKSZ;
	}
